...
# Extract results
res <- results(dds,
               contrast=c("treatment", "siADAMTS14_TGFb", "siNT_TGFb"))
...
shrunken_res <- lfcShrink(dds,
          res = res,
          coef = "treatment_siADAMTS14_TGFb_vs_siNT_TGFb", 
          type = "apeglm")
...
write.csv(res_tb, 
          file = "results/PI_cond1_vs_cond2_res_allgenes.csv",
          quote=F, row.names = F)

# Write out normalized count matrix
write.csv(counts(dds, normalized = TRUE),
          file = "results/PI_cond1_vs_cond2_normalized_counts.csv",
          quote=F)

# Write out vst matrix
write.csv(assay(rld),
          file = "results/PI_cond1_vs_cond2_vst_counts.csv",
          quote=F)
...
write.csv(left_join(norm_counts, res_tb),
          file = "results/PI_cond1_vs_cond2_res_combined_normcounts.csv",
          quote=F)
